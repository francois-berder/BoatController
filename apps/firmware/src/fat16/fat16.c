...
static int fat16_read_bpb(void)
{
    ...
    FAT16DBG("FAT16: sector count: %lu\n", bpb.sector_count);
...
int fat16_init(struct storage_dev_t _dev, uint32_t offset)
{
    ...
    FAT16DBG("FAT16: root directory sector count: %lu\n", root_directory_sector_count);
    ...
    FAT16DBG("\tstart_fat_region=%llx\n", (unsigned long long int)layout.start_fat_region);
    FAT16DBG("\tstart_root_directory_region=%llx\n", (unsigned long long int)layout.start_root_directory_region);
    FAT16DBG("\tstart_data_region=%llx\n", (unsigned long long int)layout.start_data_region);
    FAT16DBG("\tdata cluster count: %lu\n", layout.data_cluster_count);
